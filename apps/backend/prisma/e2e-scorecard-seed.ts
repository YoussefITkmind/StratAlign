...
/**
 * Deterministic Phase 3.2 E2E fixture: a real Scorecard wired to the same
 * Alignment -> KpiVersion -> StatusResult chain KPI Detail reads, so
 * scorecards.spec.ts can prove the Master Scorecard and KPI Detail render
 * identical status/color for the same underlying KPI, and a published
 * StrategyMap so the grid/map toggle has real data to render. Bypasses the
 * governance-gated mutation flow (perspective.add / weighting.publish /
 * map.publish all require an approved ApprovalCase) by writing the resulting
 * rows directly — this is seed setup, not a test of the approval workflow
 * itself, mirroring how e2e-governance-seed.ts inserts ApprovalCase rows
 * directly rather than driving the full XState submit/approve dance.
 */
...
async function main() {
  ...
  await prisma.planVersion.upsert({
    where: { id: PLAN_ID },
    update: { name: "Phase 3.2 Scorecard E2E Plan", status: "ACTIVE" },
    create: { id: PLAN_ID, name: "Phase 3.2 Scorecard E2E Plan", status: "ACTIVE" },
  });
  ...
  // Revenue is comfortably above target (On Track); CSAT is meaningfully below
  // target (Off Track) — deliberately different so the seeded scorecard shows
  // real color variation instead of a uniformly green demo.
