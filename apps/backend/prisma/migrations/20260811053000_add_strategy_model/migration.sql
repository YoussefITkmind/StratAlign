...
CREATE TABLE "strategy"."plan_versions" (
  ...
  CONSTRAINT "plan_versions_effective_range_check" CHECK ("effective_to" IS NULL OR "effective_from" IS NULL OR "effective_to" >= "effective_from")
);

CREATE UNIQUE INDEX "plan_versions_version_key" ON "strategy"."plan_versions"("version");
CREATE UNIQUE INDEX "plan_versions_one_active_idx" ON "strategy"."plan_versions" ((1)) WHERE "state" = 'active';
...
CREATE INDEX "strategy_nodes_created_by_idx" ON "strategy"."strategy_nodes"("created_by");
...
CREATE TABLE "strategy"."relationship_rules" (
  ...
  CONSTRAINT "relationship_rules_min_count_check" CHECK ("min_count" >= 0),
  CONSTRAINT "relationship_rules_max_count_check" CHECK ("max_count" IS NULL OR "max_count" > 0),
...
CREATE INDEX "owner_assignments_created_by_idx" ON "strategy"."owner_assignments"("created_by");

ALTER TABLE "strategy"."plan_versions"
  ADD CONSTRAINT "plan_versions_source_plan_version_id_fkey" FOREIGN KEY ("source_plan_version_id") REFERENCES "strategy"."plan_versions"("id") ON DELETE SET NULL ON UPDATE CASCADE,
  ADD CONSTRAINT "plan_versions_created_by_fkey" FOREIGN KEY ("created_by") REFERENCES "iam"."users"("id") ON DELETE RESTRICT ON UPDATE CASCADE;
...
ALTER TABLE "strategy"."owner_assignments"
  ...
  ADD CONSTRAINT "owner_assignments_created_by_fkey" FOREIGN KEY ("created_by") REFERENCES "iam"."users"("id") ON DELETE RESTRICT ON UPDATE CASCADE;

INSERT INTO "strategy"."relationship_rules"
  ("from_type", "to_type", "edge_type", "min_count", "max_count") VALUES
  ('corporate_strategy', 'theme', 'contains', 1, NULL),
  ('theme', 'objective', 'contains', 1, NULL),
  ('objective', 'strategic_play', 'executed_by', 0, NULL),
  ('strategic_play', 'portfolio', 'belongs_to_portfolio', 0, 1),
  ('strategic_play', 'area_of_focus', 'aligns_to', 0, NULL),
  ('objective', 'area_of_focus', 'aligns_to', 0, NULL);
...
BEGIN
  SELECT "type", "plan_version_id" INTO source_type, source_plan
    FROM "strategy"."strategy_nodes" WHERE "id" = NEW."from_node_id";
  SELECT "type", "plan_version_id" INTO target_type, target_plan
    FROM "strategy"."strategy_nodes" WHERE "id" = NEW."to_node_id";

  IF source_plan IS NULL OR target_plan IS NULL
     OR source_plan <> NEW."plan_version_id" OR target_plan <> NEW."plan_version_id" THEN
    RAISE EXCEPTION 'strategy edge nodes must belong to the same plan version';
  END IF;

  SELECT "max_count" INTO rule_max
    FROM "strategy"."relationship_rules"
    WHERE "from_type" = source_type
      AND "to_type" = target_type
...
BEGIN
  SELECT "plan_version_id" INTO node_plan
    FROM "strategy"."strategy_nodes" WHERE "id" = NEW."node_id";
