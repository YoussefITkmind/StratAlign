...
function extractText(body: OpenAiChatCompletionResponse): string {
  ...
  const [first] = body.choices as OpenAiChoice[];
 ...
 *
 * Same shape as `AnthropicLlmProvider` on purpose: dependency-free (the backend
 * already targets a Node runtime with global `fetch`), one HTTP call, failures
 * normalised before they leave this class so nothing vendor-specific reaches
 * `AiSuggestionService`.
 *
 * Requests JSON mode (`response_format: {type: "json_object"}`) instead of
 * free text — OpenAI requires the word "json" to appear somewhere in the
 * messages for that mode, which `SUGGESTION_SYSTEM_PROMPT` already satisfies
 * ("Respond with a single JSON object…"), so no prompt change was needed.
 */
