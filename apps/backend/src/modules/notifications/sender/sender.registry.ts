 ...
 *
 * In `fake` mode every channel resolves to a `FakeSender`, including IN_APP,
...
export function createSenderRegistry(
  ...
  // IN_APP has no external provider: the delivery row itself is the artefact a
  // future in-app inbox reads. The fake sender is the honest implementation of
  // "persist it and do nothing else".
  registry.register(
    new FakeSender(NotificationChannel.IN_APP, logger.child("in-app-sender")),
  );
