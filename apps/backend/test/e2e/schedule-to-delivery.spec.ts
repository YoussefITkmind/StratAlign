 ...
 * current Track C harness using the same production services and workers the
...
describe("End to end: compressed schedule through to delivery", () => {
  ...
  beforeEach(async () => {
    ...
    await seedScheduleTemplates(harness.prisma);
  });
  ...
  /**
   * An ADHOC cadence with every milestone offset set to zero and its
    ...
    return harness.services.schedulerService.createDefinition({
      ...
      payload: {
        notification: {
          ...
          recipients: [
            {
              recipientUserId: options.recipientUserId ?? "user:ada",
      ...
      cadence: { type: "ADHOC", runAt: occurrenceAt.toISOString() },
      startsAt,
      anchorAt: startsAt,
  ...
  it("drives a schedule from creation to a recorded delivery", async () => {
    ...
    const delivery = await waitFor("the review.due delivery to be sent", async () => {
      const row = await harness.prisma.notificationDelivery.findFirst({
        where: { templateKey: "schedule.review-due", recipientUserId: "user:ada" },
      });
