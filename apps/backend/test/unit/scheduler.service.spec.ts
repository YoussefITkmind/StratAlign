...
const VALID_INPUT = {
  ...
  subjectId: "kpi-42",
  cadence: { type: "DAILY", atTime: "09:00" } as const,
...
describe("SchedulerService", () => {
  ...
  describe("createDefinition", () => {
    it("stores the cadence and computes the first occurrence", async () => {
      ...
      expect(data.cadenceType).toBe("DAILY");
    ...
    it("rejects an invalid cadence configuration", async () => {
      await expect(
        service.createDefinition({
          ...VALID_INPUT,
          cadence: { type: "DAILY", atTime: "25:00" } as never,
        }),
    ...
    it("accepts a valid Arabic-region time zone", async () => {
