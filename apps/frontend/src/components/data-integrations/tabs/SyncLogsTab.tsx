...
import { LogStatus, SyncLog } from "@/types/dataIntegrations";
    ...
    <div>
      ...
      <div className="mt-5 flex flex-wrap items-center gap-2">
        ...
        <div className="flex items-center gap-1 rounded-lg bg-slate-100 p-1">
          {(["All", "Success", "Failed", "Partial", "Running"] as const).map((f) => (
            <button
              key={f}
              onClick={() => setFilter(f)}
          ...
          <tbody>
            {filtered.map((l) => (
              <tr key={l.id} className="border-b border-slate-50 last:border-0 hover:bg-slate-50/60">
                ...
                <td className="px-2 py-3">
                  <span className={`flex items-center gap-1.5 font-medium ${STATUS_STYLES[l.status]}`}>
                    <span className={`h-1.5 w-1.5 rounded-full ${DOT[l.status]}`} />
                    {l.status}
                  </span>
                </td>
