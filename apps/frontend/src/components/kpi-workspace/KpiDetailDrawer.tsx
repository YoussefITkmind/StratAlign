...
export default function KpiDetailDrawer({ row, onClose }: { row: KpiLibraryRow; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end bg-black/30" onClick={onClose}>
      <div
        className="app-scroll flex h-full w-full max-w-md flex-col overflow-y-auto bg-white shadow-xl"
          ...
          <div className="mt-4 flex gap-4 border-b border-gray-100">
            {TABS.map((t) => (
              <button
                key={t}
                onClick={() => setTab(t)}
                className={`-mb-px border-b-2 px-0.5 pb-2 text-sm font-medium uppercase tracking-wide ${
        ...
        <div className="flex-1 space-y-5 p-5">
            ...
            <>
              ...
              <div>
                ...
                <div className="rounded-xl border border-gray-100 bg-gray-50/60 p-3">
                  <TrendChart values={row.trend} favorable={row.favorable} />
                </div>
              </div>
              ...
              <dl className="space-y-3 border-t border-gray-100 pt-4">
                <DetailRow label="Owner">
                  <span className="flex items-center gap-2">
                    <span className={`flex h-6 w-6 items-center justify-center rounded-full text-[10px] font-semibold text-white ${row.owner.color}`}>
                      {row.owner.initials}
                    </span>
                    {row.owner.name}
                  </span>
                ...
                <DetailRow label="Status">
                  <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-medium ${statusMeta.bg} ${statusMeta.text}`}>
              ...
              <div className="border-t border-gray-100 pt-4">
                ...
                <p className="text-sm leading-relaxed text-gray-600">
                  {row.description ?? `${row.name} is tracked under ${row.tag} for the ${row.department} team, reported ${row.freq.toLowerCase()}.`}
                </p>
              </div>
            </>
          )}
            ...
            <div className="space-y-2">
              {row.trend.map((v, i) => (
                <div key={i} className="flex items-center justify-between rounded-lg border border-gray-100 px-3 py-2 text-sm">
                  <span className="text-gray-500">Period {i + 1}</span>
          ...
          {tab === "Approval" && (
            <div className="rounded-lg border border-gray-100 p-4 text-sm">
              ...
              <p className="mt-1 font-semibold text-gray-900">{APPROVAL_LABEL[row.approval]}</p>
            </div>
          )}
        </div>
      </div>
