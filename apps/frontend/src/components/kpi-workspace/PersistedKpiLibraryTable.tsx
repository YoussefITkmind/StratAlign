...
const OWNER_COLORS = ["bg-blue-600", "bg-emerald-600", "bg-amber-600", "bg-rose-600", "bg-cyan-600", "bg-violet-600"];
...
function CreatePersistedKpiModal({ objectives, onClose, onSaved }: { objectives: ObjectiveOption[]; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-4"><div className="w-full max-w-xl rounded-2xl bg-white shadow-2xl"><div className="flex items-center justify-between border-b p-5"><div><h2 className="text-lg font-semibold">Create KPI</h2><p className="text-xs text-gray-500">Creates a real Registry draft aligned to a strategic objective.</p></div><button type="button" onClick={onClose}><X className="h-5 w-5 text-gray-400" /></button></div><div className="space-y-4 p-5">
    ...
    <Field label="KPI Name"><input value={name} onChange={(e) => setName(e.target.value)} className="input" placeholder="e.g. Revenue Growth Rate" /></Field>
    <Field label="Strategic Objective"><select value={objectiveId} onChange={(e) => setObjectiveId(e.target.value)} className="input">{objectives.map((objective) => <option key={objective.id} value={objective.id}>{objective.department} · {objective.name}</option>)}</select></Field>
    <div className="grid grid-cols-2 gap-3"><Field label="Unit"><input value={unit} onChange={(e) => setUnit(e.target.value)} className="input" /></Field><Field label="Frequency"><select value={frequency} onChange={(e) => setFrequency(e.target.value as typeof frequency)} className="input"><option value="monthly">Monthly</option><option value="quarterly">Quarterly</option></select></Field></div>
    <div className="grid grid-cols-2 gap-3"><Field label="Polarity"><select value={polarity} onChange={(e) => setPolarity(e.target.value as typeof polarity)} className="input"><option value="higher_is_better">Higher is better</option><option value="lower_is_better">Lower is better</option></select></Field><Field label="Data Source"><select value={dataSourceType} onChange={(e) => setDataSourceType(e.target.value as typeof dataSourceType)} className="input"><option value="manual">Manual</option><option value="feed">Feed</option></select></Field></div>
    <Field label="Description"><textarea value={description} onChange={(e) => setDescription(e.target.value)} className="input min-h-20 resize-none" /></Field>
  </div><div className="flex justify-end gap-2 border-t p-4"><button type="button" onClick={onClose} className="rounded-lg border px-4 py-2 text-sm">Cancel</button><button type="button" disabled={busy || !name.trim() || !objectiveId || !session.data?.user.id} onClick={() => void save()} className="rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white disabled:opacity-40">{busy ? "Saving…" : "Create Draft"}</button></div></div></div>;
}
