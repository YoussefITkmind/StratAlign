...
import type { KpiApproval, KpiOkrWorkspaceData } from "@/types/kpi-workspace";
...
export default function KpiWorkspacePage() {
  ...
  const kpiIds = useMemo(() => workspace.kpis.map((kpi) => kpi.id), [workspace.kpis]);
  const approvalCasesQuery = trpc.governance.getLatestCasesForEntities.useQuery(
    { entityType: "KpiDefinition", entityIds: kpiIds },
    { enabled: kpiIds.length > 0 },
  );
  const governanceApprovalByKpi = useMemo(
    () => new Map(
      (approvalCasesQuery.data ?? []).map((item) => [item.entityId, item.status as KpiApproval]),
    ),
    [approvalCasesQuery.data],
  );
  const enrichedKpis = useMemo(
    () => workspace.kpis.map((kpi) => {
      if (kpi.approval === "approved") return kpi;
      const approval = governanceApprovalByKpi.get(kpi.id);
      return approval ? { ...kpi, approval } : kpi;
    }),
    [workspace.kpis, governanceApprovalByKpi],
  );
  const displayWorkspace = useMemo(
    () => ({ ...workspace, kpis: enrichedKpis }),
    [workspace, enrichedKpis],
  );
  ...
  const assistantData = useMemo(() => ({
    totalKpis: displayWorkspace.kpis.length,
    activeKpis: displayWorkspace.kpis.filter((kpi) => kpi.approval === "approved").length,
    draftKpis: displayWorkspace.kpis.filter((kpi) => kpi.approval === "draft").length,
    kpis: displayWorkspace.kpis.slice(0, MAX_ASSISTANT_CONTEXT_ITEMS).map((kpi) => ({ name: kpi.name, unit: kpi.unit, polarity: kpi.polarity, frequency: kpi.freq.toLowerCase(), status: kpi.status, actual: kpi.actual, target: kpi.target, department: kpi.department })),
    totalOkrs: displayWorkspace.okrs.length,
    okrs: displayWorkspace.okrs.slice(0, MAX_ASSISTANT_CONTEXT_ITEMS).map((okr) => ({ name: okr.title, keyResultCount: okr.keyResults.length, progressPercent: okr.progress, department: okr.department })),
    keyResults: displayWorkspace.okrs.flatMap((okr) => okr.keyResults.map((kr) => ({ okr: okr.title, title: kr.label, target: kr.target, current: kr.actual, progressPercent: kr.progress }))).slice(0, MAX_ASSISTANT_CONTEXT_ITEMS),
  }), [displayWorkspace]);
  ...
  const exportData = () => {
    const rows = activeTab === "okr" ? displayWorkspace.okrs : displayWorkspace.kpis;
