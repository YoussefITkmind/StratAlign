...
import {
  Search, Download, Plus, Link2, X, Map as MapIcon, Pencil,
} from "lucide-react";
      ...
      <div className="mb-3 flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
        <div className="flex min-w-0 flex-wrap items-center gap-2">
          ...
          {mapOrder.map((id) => {
            ...
            return (
              <div key={id} className="flex shrink-0 items-stretch overflow-hidden rounded-full border border-gray-300">
                <button
                ...
                <Link
                  href={`/strategy-maps/${id}`}
                  data-testid={`open-map-canvas-${id}`}
                  title="Open interactive canvas (edit mode)"
                  className={`flex items-center px-2.5 ${active ? "bg-slate-900 text-white/70 hover:text-white" : "bg-white text-gray-400 hover:text-blue-600"}`}
                >
