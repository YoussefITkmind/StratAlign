...
class OpenAICompatibleVLMReader:
    ...
    def extract_spm(self, image_paths: list[Path]) -> ExtractedSPMDocument:
        ...
        prompt = (
            ...
            "Keep objectives, KPIs and initiatives semantically distinct. "
            "OBJECTIVES: extract items explicitly presented as strategic objectives, strategic priorities, goals, or objective-health areas. "
            "If an objective-health chart or scorecard visibly lists named objective areas, preserve those areas as objectives even when owner/status details are absent. "
            "When a detailed objective heading clearly corresponds to a shorter summary label (for example 'Improve Customer Experience' and 'Customer Experience'), prefer the detailed objective as the canonical name and use the shorter label only as an alias; do not emit both as separate objectives. "
            "KPIS: a KPI must be a measurable performance indicator with an explicitly visible metric, target, actual, status, unit or measurement context. "
            "Exclude executive dashboard aggregates and roll-up counters that summarize other objectives/KPIs rather than measuring a business outcome directly, such as overall strategic health scores, objectives-on-track counts, KPIs-at/above-target counts, and counts of critical risks. "
            "Financial measures may be KPIs when the document presents an actual/budget/target/variance measure. "
            "Do not turn management decisions, recommendations, risks, risk counts, narrative observations, initiatives, milestones or objective statements into KPIs. "
            "Do not emit duplicate entities just because the same item appears in an executive summary and a detail section; prefer the most complete visible representation. "
            "For KPIs, classify category as kpi, financial, risk, operational, people, or other; capture reporting period when explicitly visible; include aliases only when the document establishes them. "
