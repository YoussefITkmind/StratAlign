...
export interface PerformanceServiceContract {
  ...
  validateCaptureTemplate(bytes: Buffer, format: "csv" | "xlsx", expectedPeriod: string, history: number[], sessionId?: string, ownerId?: string): Promise<Array<{ row: number; period: string; value: number | null; outcome: "accepted" | "rejected" | "warning"; reason?: string }>>;
...
export const appRouter = router({
  ...
  performance: router({
    ...
    capture: router({
      ...
      validateTemplate: requireRole("kpi_owner", "data_steward").input(z.object({ base64: z.string().max(14_000_000), format: z.enum(["csv", "xlsx"]), expectedPeriod: periodSchema, history: z.array(z.number().finite()).max(100), sessionId: z.string().uuid().optional() }).strict()).output(z.array(bulkRowSchema)).mutation(({ ctx, input }) => ctx.performance.validateCaptureTemplate(Buffer.from(input.base64, "base64"), input.format, input.expectedPeriod, input.history, input.sessionId, ctx.session.user.id) as never),
