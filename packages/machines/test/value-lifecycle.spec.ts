...
function actor(input?: Partial<{ baselineExists: boolean; realizedEntryCount: number }>) {
...
describe("value lifecycle machine", () => {
  it("composes the generic ApprovalCase for the pending approval step", () => {
    const service = actor();
  ...
  it("does not leave approved until a baseline exists", () => {
    ...
    expect(service.getSnapshot().value).toBe("approved");
    service.send({ type: "REFRESH_FACTS", baselineExists: true, realizedEntryCount: 0 });
    service.send({ type: "BEGIN_VALIDATION" });
    expect(service.getSnapshot().value).toBe("validation");
  });

  it("does not close tracking until at least one realized entry exists", () => {
    ...
    service.send({ type: "REFRESH_FACTS", baselineExists: true, realizedEntryCount: 1 });
    service.send({ type: "CLOSE", stopReason: "realization complete" });
